...
function toString(fields) {
	...
	fields.forEach(function(field) {
		var data = _(field.subfield).pluck('_');
...
function lowercase(arrayOfStrings) {
...
function trimEnd(fields, options) {
	return replace(new RegExp(/\s*$/))(fields, options);
}
function trim(fields, options) {
	applyToFieldValues(fields, function(content) {
		return content.trim();
	}, options);
	return fields;
}

function sortContent(fields, options) {
	applyToFieldValues(fields, function(fieldContent) {
		return fieldContent.split(' ').sort().join(' ');
	}, options);
	
	return fields;
}

function upper(fields, options) {
	applyToFieldValues(fields, function(fieldContent) {
		return fieldContent.toUpperCase();
	}, options);
	
	return fields;
}

function removediacs(fields, options) {
	applyToFieldValues(fields, removeDiacritics, options);
	return fields;
}
function utf8norm(fields, options) {
	applyToFieldValues(fields, utf8normString, options);
	return fields;
}
function romanToArabic(fields, options) {
	applyToFieldValues(fields, romanToArabicConversion, options);
	return fields;
}

function romanToArabicConversion(str) {
	return str.split(' ').map(asArabic).join(' ');
...
function collapse(fields, options) {
	var re = new RegExp("\\s+","g");

	applyToFieldValues(fields, function(fieldContent) {
		return fieldContent.replace(re, ' ');
	}, options);
...
function onlyNumbers(fields, options) {
	...
	applyToFieldValues(fields, function(fieldContent) {
		return fieldContent.replace(/\D/g,' ').replace(/\s+/g,' ');
	}, options);
...
function removeEmpty(fields, options) {
	...
	fields = fields.filter(function(field) {
		return field.subfield.length > 0;
	});
	return fields;
}
...
function applyToFieldValues(fields, func, options) {
	...
	fields.forEach(function(field) {
		if (field.subfield !== undefined) {
			...
			field._ = func(field._);
		}
	});
}

function toSpace(stringOfChars) {
	var arrayOfChars = stringOfChars.split('');

	return function(fields, options) {
		applyToFieldValues(fields, function(fieldContent) {
			arrayOfChars.forEach(function(c) {
				fieldContent = fieldContent.replace(new RegExp(RegExp.escape(c), "g"),' ');
			});
			return fieldContent;
		}, options);
		return fields;
...
function delChars(stringOfChars) {
	var arrayOfChars = stringOfChars.split('');
	return function(fields, options) {
		applyToFieldValues(fields, function(fieldContent) {
			arrayOfChars.forEach(function(c) {
...
function replace(regexp, to) {
	...
	return function(fields, options) {
		...
		applyToFieldValues(fields, function(fieldContent) {
			...
			return fieldContent.replace(regexp, to);
...
function fieldToString(field) {
	var ind1 = field.$.ind1 !== ' ' ? field.$.ind1 : '_';
	var ind2 = field.$.ind2 !== ' ' ? field.$.ind2 : '_';
	
	// Indicators are always undefined for controlfields.
	ind1 = ind1 || ' ';
	ind2 = ind2 || ' ';
...
module.exports = {
	...
	removediacs: removediacs,
	utf8norm: utf8norm,
	toSpace: toSpace,
	collapse: collapse,
	romanToArabic: romanToArabic,
	applyToFieldValues: applyToFieldValues,
